...
class Point(object):
    ...
    def __add__(self, other):
        ...
        p = self.__curve.p()
        lOne = ((other.__y - self.__y) * inverse_mod(other.__x - self.__x, p)) % p
        x3 = (lOne * lOne - self.__x - other.__x) % p
        y3 = (lOne * (self.__x - x3) - self.__y) % p
    ...
    def double(self):
        ...
        p = self.__curve.p()
        a = self.__curve.a()
        lOne = ((3 * self.__x * self.__x + a) * inverse_mod(2 * self.__y, p)) % p
        x3 = (lOne * lOne - 2 * self.__x) % p
        y3 = (lOne * (self.__x - x3) - self.__y) % p
        return Point(self.__curve, x3, y3)
...
if __name__ == "__main__":
    ...
    # print (privkey)
    if VERBOSE >= 1:
        print('secret = ' + "{0:0{1}x}".format(secret, 64))
